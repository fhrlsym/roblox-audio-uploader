...
export default function Home() {
  ...
  const [activeTab, setActiveTab] = useState<'convert' | 'upload' | 'history'>('convert');
      ...
      <div className="relative mx-auto max-w-5xl px-4 pb-16 pt-8 md:px-6">
        ...
        <div className="mt-6 flex flex-wrap items-center gap-3">
          <button
            ...
            className="flex items-center gap-2 rounded-xl border border-[#d4af37]/25 px-4 py-2 text-sm text-[#e6c15c]/80 transition-colors hover:bg-[#d4af37]/10 hover:text-[#f5d06f] active:scale-[0.97]"
          >
        ...
        <div className="mt-6 flex gap-2 border-b border-white/10">
          ...
          ] as { id: 'convert' | 'upload' | 'history'; label: string; icon: string }[]).map((tab) => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              className={`flex items-center gap-2 border-b-2 px-4 py-3 text-sm font-medium transition-colors active:scale-[0.97] ${
                activeTab === tab.id
                  ? 'border-[#f5d06f] text-[#f5d06f]'
        ...
        <main className="mt-6">
            ...
            <div className="space-y-6">
              ...
              <section className={`${CARD} p-6`}>
                <div className="mb-4 flex items-center justify-between">
                  <h3 className="text-sm font-medium uppercase tracking-wider text-white/60">YouTube Converter</h3>
                  <button
                  ...
                  >
                    <span className={`h-2 w-2 rounded-full ${autoUpload ? 'bg-[#f5d06f]' : 'bg-white/25'}`} />
                ...
                {youtubeLinks.length > 0 && (
                  <>
                    <div className="mt-3 flex items-center justify-between text-xs">
                      <span className="text-white/40">{youtubeLinks.length} link</span>
                      <button
                    ...
                    <div className="mt-3 max-h-40 space-y-2 overflow-y-auto pr-1">
                      {youtubeLinks.map((link, index) => (
                        <div key={index} className="flex items-center gap-2 rounded-lg border border-white/10 bg-black/40 px-3 py-2">
                          <span className="min-w-0 flex-1 truncate text-xs text-white/70">{link}</span>
                          <button
                            onClick={() => removeYoutubeLink(index)}
                            disabled={downloading}
                            className="shrink-0 text-xs text-white/50 transition-colors hover:text-rose-300 disabled:opacity-50"
                          >
                            ✕
                          </button>
                        </div>
                ...
                {downloadProgress.length > 0 && (
                  <div className="mt-4 max-h-60 space-y-2 overflow-y-auto pr-1">
                    {downloadProgress.map((item, index) => (
                      <div key={index} className="stagger-enter flex items-center gap-2 rounded-lg border border-white/10 bg-black/40 px-3 py-2 text-xs" style={{ animationDelay: `${Math.min(index * 40, 320)}ms` }}>
                        ...
                        <span className={item.status === 'failed' ? 'text-rose-300/80' : 'text-white/40'}>
                ...
                <details className="mt-4 rounded-lg border border-white/10 bg-black/30">
                  <summary className="cursor-pointer px-3 py-2 text-xs text-white/50 transition-colors hover:text-white">
                    YouTube Cookies <span className="text-white/30">(opsional)</span>
                  </summary>
              ...
              {autoUpload && results.length > 0 && (
                <section className={`${CARD} p-6`}>
                  <div className="mb-4 flex items-center justify-between">
                    <h3 className="text-sm font-medium uppercase tracking-wider text-white/60">Hasil Upload</h3>
                    <button onClick={copyResults} className="text-xs text-[#e6c15c]/80 transition-colors hover:text-[#f5d06f]">
                      Salin
                    </button>
                  </div>
                        ...
                        {result.success ? (
                          <>
                            <div className="flex items-start justify-between gap-2">
                              <div className="min-w-0 flex-1 truncate text-sm font-medium text-white">{result.filename}</div>
                              <StatusBadge status={result.status} />
                            </div>
                            <div className="mt-2 flex items-center gap-2 text-xs">
                              <span className="text-white/40">ID:</span>
                              <span className="font-mono text-white/70">{result.assetId}</span>
                              <button
                                onClick={() => copyAssetId(result.assetId)}
                                className="ml-auto text-[#e6c15c]/80 transition-colors hover:text-[#f5d06f]"
                              >
                                Copy
                              </button>
                            </div>
          ...
          {activeTab === 'upload' && (
            <div className="space-y-6">
              {/* Upload Files */}
              <section className={`${CARD} p-6`}>
                <h3 className="mb-4 text-sm font-medium uppercase tracking-wider text-white/60">Upload Files</h3>
                <div
                  onDrop={handleDrop}
                  onDragOver={(e) => e.preventDefault()}
                  onClick={() => document.getElementById('fileInput')?.click()}
                  className="cursor-pointer rounded-xl border border-dashed border-[#d4af37]/25 px-6 py-8 text-center transition-colors hover:border-[#d4af37]/50 hover:bg-[#d4af37]/[0.03]"
                >
                ...
                {files.length > 0 && (
                  <>
                    <div className="mt-3 flex items-center justify-between text-xs">
                      <span className="text-white/40">{files.length} file</span>
                      <button
                        onClick={() => setFiles([])}
                      ...
                      {files.map((file, index) => (
                        <div key={index} className="flex items-center gap-2 rounded-lg border border-white/10 bg-black/40 px-3 py-2">
                          ...
                          <button
                            ...
                            className="shrink-0 text-xs text-white/50 transition-colors hover:text-rose-300"
                          >
                            ✕
                          </button>
                        </div>
              ...
              <section className={`${CARD} p-6`}>
                <h3 className="mb-4 text-sm font-medium uppercase tracking-wider text-white/60">Roblox Account</h3>
                
                <div className="space-y-4">
                  {/* User/Group Toggle */}
                  <div className="grid grid-cols-2 gap-2 rounded-lg border border-[#d4af37]/20 bg-black/40 p-1">
                  ...
                  {/* ID Input */}
                  <div>
                    <label className="mb-2 block text-xs text-white/40">{targetType === 'user' ? 'User ID' : 'Group ID'}</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <div className="mb-2 flex items-center justify-between">
                      <label className="text-xs text-white/40">API Keys</label>
                      <button onClick={addApiKeyField} className="text-xs text-[#e6c15c] transition-colors hover:text-[#f5d06f]">
                        + Add key
                      </button>
                    </div>
                        ...
                        <div key={index} className="flex gap-2">
                          ...
                          {apiKeys.length > 1 && (
                            <button
                              ...
                              className="shrink-0 rounded-lg border border-white/10 px-3 text-xs text-white/50 transition-colors hover:border-rose-400/30 hover:text-rose-300"
                            >
                              ✕
                            </button>
                          )}
                ...
                <button
                  ...
                  className="mt-6 w-full rounded-xl bg-gradient-to-r from-[#f5d06f] via-[#e6c15c] to-[#c9a227] py-4 text-base font-bold text-black shadow-[0_0_30px_rgba(212,175,55,0.2)] transition-transform duration-150 hover:brightness-110 active:scale-[0.98] disabled:cursor-not-allowed disabled:from-white/10 disabled:via-white/10 disabled:to-white/10 disabled:text-white/40 disabled:shadow-none disabled:active:scale-100"
                >
          ...
          {activeTab === 'history' && (
            <section className={`${CARD} p-6`}>
              <div className="mb-4 flex items-center justify-between">
                <h3 className="text-sm font-medium uppercase tracking-wider text-white/60">Riwayat Upload</h3>
                <button
                  onClick={() => loadUploadHistory()}
                  className="text-xs text-[#e6c15c]/80 transition-colors hover:text-[#f5d06f]"
                >
                  Refresh
                </button>
              </div>
              ...
              {uploadHistory.length === 0 ? (
                <p className="py-8 text-center text-sm text-white/40">Belum ada history</p>
              ) : (
                <div className="max-h-[60vh] space-y-2 overflow-y-auto pr-1">
                  {uploadHistory.map((item) => (
                    <div key={item.id} className="rounded-lg border border-white/10 bg-black/40 px-4 py-3">
                        ...
                        <div className="flex shrink-0 items-center gap-2">
                          ...
                          {item.status === 'Pending' && (
                            <button
                              ...
                              className="rounded-lg border border-[#d4af37]/25 px-2 py-1 text-xs text-[#e6c15c]/80 transition-colors hover:bg-[#d4af37]/10 hover:text-[#f5d06f]"
                            >
                              Refresh
                            </button>
                          )}
        ...
        {showSettings && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4" onClick={() => setShowSettings(false)}>
            <div className="modal-enter w-full max-w-md rounded-2xl border border-[#d4af37]/25 bg-[#0d0d10] p-6" onClick={(e) => e.stopPropagation()}>
              <div className="mb-4 flex items-center justify-between">
                <h3 className="text-lg font-semibold text-[#f5d06f]">Audio Settings</h3>
                <button onClick={() => setShowSettings(false)} className="text-white/40 transition-colors hover:text-white active:scale-95">✕</button>
              </div>

              <div className="space-y-4">
                <div>
                  <div className="mb-2 flex items-center justify-between">
                    <label className="text-xs text-white/40">Speed (Playback)</label>
                    <span className="text-xs text-white/25">makin besar = cepat</span>
                  </div>
                  <input
                    type="number"
                    step="0.01"
                ...
                <div>
                  <div className="mb-2 flex items-center justify-between">
                    <label className="text-xs text-white/40">Amplify (dB)</label>
                    <span className="text-xs text-white/25">minus = pelan</span>
                  </div>
                  <input
                    type="number"
                    step="1"
                ...
                <div className="rounded-lg border border-[#d4af37]/25 bg-gradient-to-br from-[#d4af37]/[0.12] to-transparent px-4 py-3">
                  <div className="text-xs uppercase tracking-wider text-[#e6c15c]/70">Roblox Playback</div>
                  <div className="mt-1 font-mono text-2xl tabular-nums text-[#f5d06f]">{calculateRobloxPlaybackSpeed()}</div>
                </div>

                <button
                  onClick={() => { setSpeed(2.30); setAmplify(-4); }}
                  className="w-full rounded-lg border border-white/10 py-2 text-xs text-white/50 transition-colors hover:border-[#d4af37]/30 hover:text-[#e6c15c] active:scale-95"
                >
                  Reset ke bawaan
                </button>
              </div>
